...
class Shopware_Plugins_Frontend_ZerintPaymentBarzahlen_Bootstrap extends Shopware_Components_Plugin_Bootstrap
{
    ...
    protected function createEvents()
    {
        ...
        $event = $this->createEvent('Enlight_Controller_Action_PostDispatch_Backend_Order', 'onGetOrderControllerPostDispatch');
        $this->subscribeEvent($event);

        $event = $this->createEvent('Enlight_Controller_Action_PreDispatch_Backend_Order', 'onGetOrderControllerPreDispatch');
        $this->subscribeEvent($event);
    ...
    public function onBackendIndexPostDispatch(Enlight_Event_EventArgs $args)
    {
        ...
        if (Shopware()->Auth()->hasIdentity() && ($lastCheck == 0 || $lastCheck < strtotime("-1 week"))) {
            ...
            try {
                ...
                $response = $checker->checkVersion('Shopware 4', Shopware::VERSION, $this->getVersion());
